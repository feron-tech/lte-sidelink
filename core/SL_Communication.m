    ...
    methods
        ...
        function h = SL_Communication(varargin)
            ...
            if (isequal(h.slMode,3) || isequal(h.slMode,4))
               fprintf('Currently V2X sidelink communication is not fully supported\n'); keyboard;
            end
            ...
            if isequal(h.slMode,1)
                ...
                % assertion for ITRP at a later stage (in GetResourcesPerUE)
            ...
            elseif isequal(h.slMode,3) || isequal(h.slMode,4) % v2x: not fully supported yet
                % 36.212/5.4.3.1.2
                %h.sci1_TBsize = 32;
                % XXX
        ...
        function [output_seq, d_seq] = SL_SCI_PSCCH_Encode(h, input_seq)
            ...
            if isequal(h.slMode,3) || isequal(h.slMode,4)
                % h.nXID = bitTodec(b_seq(length(a_seq)+1:end),true);
            end
        ...
        function [output_seq, CRCerror_flg, d_seq_rec] = SL_SCI_PSCCH_Recover(h, input_seq, decodingType)
           ...
           elseif isequal(decodingType,'Soft')
            ...
            % for v2x compute nXID (36.211/9.3.1)
            if isequal(h.slMode,3) || isequal(h.slMode,4)
                %h.nXID = bitTodec(b_seq_rec(length(a_seq_rec)+1:end),true);
            end
        ...
        function output_seq = PSSCH_Generate(h, input_seq, nssfPSSCH, userIndex)
            ...
            %elseif isequal(h.slMode, 3) || isequal(h.slMode, 4),     pssch_c_init = h.nXID*2^14 + nssfPSSCH*2^9 + 510;
            end
        ...
        function output_seq = CreateSubframe (h, subframe_counter)
            ...
            %Create a sidelink communications subframe (PSCCH or PSSCH) for a given subframe index
